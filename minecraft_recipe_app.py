...
def create_recipe():
    ...
    rec_output_count = int(input("Enter the number of items produced by this recipe (default 1): ") or "1")
    ...
    while True:
        ...
        if choice == 'done':
            ...
            if existing_recipes:
                ...
                selected_recipe_id = int(input("Select the ID of the recipe to use as an ingredient: "))
                # Find the selected recipe details
                selected_recipe = next((name for id, name, count in existing_recipes if id == selected_recipe_id), None)
                if selected_recipe:
                    final_quantity_needed = int(input(f"Enter the quantity of {selected_recipe} needed: "))
                    nested_recipes[selected_recipe_id] = final_quantity_needed  # Store the final product quantity
                else:
...
def calculate_ingredients(recipe_name, desired_quantity):
    def calculate(recipe, multiplier):
        ...
        for ingredient, quantity in recipe.ingredients.items():
            ...
            ingredients_needed[ingredient] = ingredients_needed.get(ingredient, 0) + total_quantity
        ...
        for nested_id, final_quantity_needed in recipe.nested_recipes.items():
            ...
            if nested_recipe:
                nested_multiplier = math.ceil(final_quantity_needed / nested_recipe.output_count)
                nested_ingredients, nested_steps = calculate(nested_recipe, nested_multiplier)
                steps.append((nested_recipe.name, nested_multiplier, nested_recipe.output_count, nested_steps))
    ...
    def print_steps(steps):
        for step_name, step_multiplier, step_output, nested_steps in steps:
            print(f"- {step_multiplier} Recipe {step_name} (Output: {step_output}, " +
                  ", ".join([f"{qty} {ing}" for ing, qty in db.fetch_recipe(step_name).ingredients.items()]) + ")")
